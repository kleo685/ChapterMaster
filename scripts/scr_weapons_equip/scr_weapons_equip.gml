...
function scr_weapons_equip() {
	...
	if (equipment_slot = 3) and(unit_type == unit_type_astartes) { // unit_type == unit_type_astartes for normal infantry gear
		...
		if (!instance_exists(obj_creation)) and(!instance_exists(obj_mass_equip)) {
			...
			var bub = 0;
			if (instance_exists(obj_creation)) {
				if (type = 112) then bub = 1;
			}
			if (instance_exists(obj_mass_equip)) {
				if (obj_controller.settings = 12) then bub = 1;
			}
			i += 1;
			item_name[i] = "(None)";
			i += 1;
			item_name[i] = "Scout Armour";
			// i+=1;item_name[i]="Power Armour";
			i += 1;
			item_name[i] = "MK3 Iron Armour";
			i += 1;
			item_name[i] = "MK4 Maximus";
			i += 1;
			item_name[i] = "MK5 Heresy";
			i += 1;
			item_name[i] = "MK6 Corvus";
			i += 1;
			item_name[i] = "MK7 Aquila";
			i += 1;
			item_name[i] = "MK8 Errant";
			i += 1;
			item_name[i] = "Artificer Armour";
			i += 1;
			item_name[i] = "Terminator Armour";
			i += 1;
			item_name[i] = "Tartaros";
		}
	...
	if (equipment_slot = 4) and(unit_type == unit_type_astartes || unit_type == unit_type_terminator) { // unit_type == unit_type_astartes for normal infantry gear
		if (!instance_exists(obj_creation)) and(!instance_exists(obj_mass_equip)) {
			...
			i += 1;
			item_name[i] = "Combat Shield";
			i += 1;
			item_name[i] = "Iron Halo";
			i += 1;
			item_name[i] = "Narthecium";
			i += 1;
			item_name[i] = "Psychic Hood";
			i += 1;
			item_name[i] = "Rosarius";
	...
	if (equipment_slot = 5) and(unit_type == unit_type_astartes) { // unit_type == unit_type_astartes for normal infantry gear
		if (!instance_exists(obj_creation)) and(!instance_exists(obj_mass_equip)) {
			...
			item_name[i] = "Bike";
			i += 1;
			item_name[i] = "Jump Pack";
			i += 1;
			item_name[i] = "Servo-arm";
			i += 1;
			item_name[i] = "Servo-harness";
		}
	}
	...
	if (equipment_slot = 5) and(equipment_tab = 1) and((unit_type == unit_type_land_raider) or(unit_type == unit_type_rhino) or(unit_type == unit_type_predator) or(unit_type == unit_type_whirlwind) or(unit_type == unit_type_dreadnought)) {
		var i;
		i = 0; // Tank Accessory
		i += 1;
		item_name[i] = "(None)";
		if unit_type != unit_type_dreadnought {
			i += 1;
		...
		if !unit_is_vehicle and unit_type != unit_type_dreadnought {
			i += 1;
