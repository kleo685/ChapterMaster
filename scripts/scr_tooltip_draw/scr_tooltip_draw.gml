function tooltip_draw(_tooltip="", _width=350, _coords=[mouse_x+20,mouse_y+20], _text_color=#50a076, _font=fnt_40k_14, _header="", _header_font=fnt_40k_14b, _force_width=false){
	...
	if (_rect_y + _rect_h > yy + __view_get(e__VW.HView, 0) - 60) {
		...
		// Check if the tooltip is now above the top edge of the screen and clamp it if so
		if (_rect_y < yy + 40) {
			_rect_y = yy + 40;
		}
	}
...
function draw_attack_tooltip(_tooltip=[], _width=350, _coords=[mouse_x+20,mouse_y+20], _text_color=#50a076, _font=fnt_40k_14, _header="", _header_font=fnt_40k_14b, _force_width=false){
	// Remember global variables
	var _curr_font = draw_get_font();
	var _curr_color = draw_get_color();
	var _curr_halign = draw_get_halign();
